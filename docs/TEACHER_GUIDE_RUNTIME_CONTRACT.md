...
## TDE 11 Teacher Guide V3 runtime

The materialized TDE_11 package advertises `book_first_v3` as the current
textbook-first Teacher Guide projection. Its architecture version is `3.0.0`
and its projection version is `1.0.0+textbook-first-v3-snapshot`. The runtime
is source-bound to TYMM commit
`bb345b0c51074aa351b1b88b6b889cebf6a2af48`.

The release contract currently requires 4 theme guides, 28 sections, 431
units, 576 guide items, 407 `QUESTION` items, zero `LOCATOR_ONLY` questions,
and clean SQLite foreign keys. `book_first_v23` may remain in manifest metadata
as a compatibility alias for older application code, but its architecture and
projection metadata point to the same V3 materialization. The authoritative
capability for new code is `book_first_v3`.

The V3 database is generated by `tool/run_teacher_guide_v3_projection.py` and
validated before it may replace the checked-in TDE_11 `course_runtime.sqlite`.
A generated runtime commit made by GitHub Actions does not recursively trigger
another workflow run, so release builds must be triggered by a user-authored
`main` push (or `workflow_dispatch`) after a newly materialized runtime is
published.
