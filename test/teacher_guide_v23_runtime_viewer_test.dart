...
void main() {
  ...
  test('real TDE11 V2.3 runtime exposes deterministic printed-page targets via bulk read', () async {
    ...
    expect(
      items.every(
        (item) => item.provenance.contentClass == 'BOOK_FIRST_V2_3_ITEM',
      ),
      isTrue,
    );
    ...
    expect(
      questions.every(
        (item) => const {
          'VERBATIM_SHORT',
          'VERIFIED_SUMMARY',
        }.contains(item.provenance.additional['prompt_mode']),
      ),
      isTrue,
    );
  ...
  test('real visual-dependent question remains source-bound without an invented answer', () async {
    final fixture = await _openRuntime();
    addTearDown(fixture.database.close);
